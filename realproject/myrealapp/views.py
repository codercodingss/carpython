...
def index(request):
    ...
    testimonials = Testimonials.objects.all()  # Fetch all testimonials
...
def customer_edit(request):
    if request.method == 'POST':
        ...
        try:
            ...
            customer = Customer.objects.create(user=user, phone=phone, location=location, type="Customer")

        alert = True
        return render(request, "customer_homepage.html", {'alert': alert})

    return render(request, "customer_edit.html", {'user': request.user})
...
def add_car(request):
    if request.method == "POST":
        ...
        if location is not None:
            ...
            car.save()
        alert = True
        return render(request, "add_cars.html", {'alert':alert})
...
def testimonials(request):
    if request.method == "POST":
        ...
        city = request.POST['city']
        image = request.FILES['image']
        dmessage = request.POST['dmessage']
        try:
            location = Location.objects.get(city=city)
            location.save()
        except:
            location = None
        if location is not None:
            testimonials = Testimonials(name=fname, location=location, image=image, message=dmessage)
            testimonials.save()
        else:
            location = Location(city=city)
            location.save()
            ...
            testimonials.save()
        alert = True
        return render(request, "testimonials.html", {'alert':alert})
